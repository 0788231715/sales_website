  ...
  return (
    <div className="py-24 px-6 max-w-7xl mx-auto min-h-screen bg-background">
      <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
        ...
        >
          <h1 className="text-4xl font-serif mb-2 text-foreground">Our Properties</h1>
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="relative flex-grow md:w-64">
            ...
            <input 
              type="text" 
              placeholder="Search..." 
              className="w-full pl-12 pr-4 py-3 bg-foreground/5 border border-foreground/10 rounded-xl focus:border-accent outline-none text-foreground"
            />
          </div>
          <button className="p-4 bg-foreground/5 border border-foreground/10 rounded-xl hover:text-accent transition-colors text-foreground/60">
            <FiFilter />
          </button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {properties.length > 0 ? properties.map((property: any, i: number) => (
            <motion.div
              ...
              whileHover={{ y: -10 }}
              className="group relative bg-background rounded-3xl overflow-hidden border border-foreground/10 shadow-lg hover:shadow-2xl transition-all duration-500"
            >
              <Link href={`/properties/${property.id}`}>
                <div className="relative h-72 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 flex gap-2">
                    <motion.div 
                      className={cn(
                        "text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-tighter shadow-lg",
                    ...
                    {property.views_count > 100 && (
                      <motion.div 
                        ...
                        className="bg-accent text-primary-dark text-[10px] font-bold px-3 py-1 rounded-full flex items-center gap-1 shadow-lg"
                      >
                        <FiTrendingUp /> HOT
                      </motion.div>
                    )}
                ...
                <div className="p-6">
                  <h4 className="text-xl font-serif mb-2 text-foreground group-hover:text-accent transition-colors">{property.title}</h4>
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-foreground/10">
                    <div className="flex gap-4 text-foreground/80">
                      ...
                      <span className="text-sm font-medium">{property.size} sqm</span>
                    </div>
                  </div>
                </div>
              </Link>
